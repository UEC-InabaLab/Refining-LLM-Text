...
def main():
    ...
    rec_llm = RecModel("dpo-recommendation-results-new")
    sum_llm = SummaryModel("dpo-summary-results-new")
    input_dir = "../data/datasets_1/"
    # output_dir = '../data/recommend_data_proposal/'
    output_dir = "../data/recommend_data_proposal/"
    # 出力ディレクトリが存在しない場合は作成
...
def many_main():
    ...
    for i in range(4):
        ...
        sum_llm = SummaryModel(model_name=f"dpo-summary-results-new_{i+2}")
        input_dir = "../data/datasets_1/"
        output_dir = f"../data/recommend_data_proposal_{i+2}/"
        # 出力ディレクトリが存在しない場合は作成
        os.makedirs(output_dir, exist_ok=True)
        for filename in tqdm(os.listdir(input_dir)):
            print(f"filename:{filename}")
