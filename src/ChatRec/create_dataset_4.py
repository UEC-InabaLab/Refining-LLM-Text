...
def create_formatted_data(llm, data: dict) -> list:
    ...
    for rec_info, score in zip(candidates_infos, A_Scores):
        if score == 1:
            ...
            rec_sentence_list = [
                llm.generate_rec_sentence_with_no_dialogue(rec_info) for _ in range(10)
            ]
            data = create_preference_data(
                user_name="A",
                dialogue_summary=generated_summary,
                rec_sentence_list=rec_sentence_list,
                candidate_information=rec_info,
                score=score,
            )
            datas.append(data)
...
def create_datasets_4():
    ...
    output_dir = os.path.join(BASE_DIR, "../../data/ChatRec/datasets_4")
    ...
    for dir_name in tqdm(target_dirs, desc="Processing directories"):
        ...
        for filename in tqdm(
            os.listdir(dir_path), desc=f"Processing files in {dir_name}"
        ):
