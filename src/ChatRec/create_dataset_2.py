    ...
    def process_data(self, input_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for place in places:
            ...
            if place_name not in recommendation_cache:
                recommendation_cache[place_name] = (
                    self.llm.generate_rec_sentence_with_no_dialogue(rec_info=place)
                )
...
def process_all_files(base_dir: str, output_dir: str, subsets: List[str]):
    ...
    try:
        ...
        for subset in subsets:
            for subset_dir in [
                f"{subset}-except_for_travel",
                f"{subset}-no_restriction",
                f"{subset}-travel",
            ]:
                ...
                if os.path.exists(src_dir):
                    ...
                    all_json_files.extend(
                        [(json_file, subset, subset_dir) for json_file in json_files]
                    )
        ...
        for json_file, subset, subset_dir in progress_bar:
            ...
            try:
                ...
                if os.path.exists(output_file):
                    ...
                    category = subset_dir.split("-", 1)[
                        1
                    ]  # except_for_travel, no_restriction, travelの部分を抽出
                    output_file = os.path.join(
                        subset_output_dir, f"{base_name}_{category}{ext}"
                    )
                ...
                progress_bar.set_postfix(
                    {"places": places_count, "entries": entries_count}
                )
        ...
        wandb.log(
            {
                ...
                "success_rate": (
                    (processed_count - error_count) / total_files * 100
                    if total_files > 0
                    else 0
                ),
            }
        ...
        wandb.alert(
            title="Critical Error", text=error_message, level=wandb.AlertLevel.ERROR
        )

        # エラー情報をログ
        wandb.log({"critical_error": True, "error_message": error_message})
...
def main():
    ...
    """
    # 入力データのベースディレクトリ
    base_dir = os.path.join(BASE_DIR, "../../data/ChatRec/datasets_1")

    # 出力先のディレクトリ
    output_dir = os.path.join(BASE_DIR, "../../data/ChatRec/datasets_2")
