...
def create_formatted_data(data, user):
    ...
    return {"predicted_score": predicted_scores, "score": scores}


def create_recommend_data(data, llm):
    """
    ...
    # 会話の要約をユーザーごとに生成
    dialogue_summary_A = llm.generate_summary(data["dialogue"], "A")
    dialogue_summary_B = llm.generate_summary(data["dialogue"], "B")
    ...
    for candidate_info in candidates_info_list:
        # ユーザーAの予測スコア生成
        recommend_sentence_A = llm.generate_rec_sentence_with_no_dialogue(
            candidate_info, temperature=0.2
        )
        predicted_score_A = predict_score_with_deberta(
            dialogue_summary_A, recommend_sentence_A, candidate_info
        )
        predicted_scores_A.append(predicted_score_A)

        # ユーザーBの予測スコア生成
        recommend_sentence_B = llm.generate_rec_sentence_with_no_dialogue(
            candidate_info, temperature=0.2
        )
        predicted_score_B = predict_score_with_deberta(
            dialogue_summary_B, recommend_sentence_B, candidate_info
        )
...
def main():
    ...
    input_dir = os.path.join(BASE_DIR, "../../data/ChatRec/datasets_1/")
    output_dir = os.path.join(BASE_DIR, "../../data/ChatRec/recommend_data_baseline1/")
    ...
    for dir_name in os.listdir(input_dir):
        ...
        for filename in tqdm(os.listdir(test_dir)):
            ...
            with open(input_file_path, "r", encoding="utf-8") as file:
                data = json.load(file)
                output_data = create_recommend_data(data, llm)
...
if __name__ == "__main__":
    wandb.init(project="recommend_data_baseline1", job_type="baseline1")
    try:
        main()
        # 正常終了時の alert
        wandb.alert(
            title="recommend_data_baseline1 Creation Finished",
            text="The dataset creation process has completed successfully.",
        )
    except Exception as e:
        # エラー発生時の alert
        wandb.alert(
            title="recommend_data_baseline1 Creation Error",
