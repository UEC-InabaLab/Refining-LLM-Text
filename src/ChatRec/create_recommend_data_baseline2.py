...
from inference_deberta import predict_score_with_deberta_baseline2
import time
import wandb
import socket
...
def create_recommend_data(data, llm):
    ...
    for candidate_info in candidates_info_list:
        # ユーザーAの予測スコア生成
        predicted_score_A = predict_score_with_deberta_baseline2(
            dialogue_summary=dialogue_summary_A, rec_info=candidate_info
        )
        predicted_scores_A.append(predicted_score_A)

        # ユーザーBの予測スコア生成
        predicted_score_B = predict_score_with_deberta_baseline2(
            dialogue_summary=dialogue_summary_B, rec_info=candidate_info
        )
...
def main():
    ...
    output_dir = os.path.join(BASE_DIR, "../../data/ChatRec/recommend_data_baseline2/")
...
if __name__ == "__main__":
    wandb.init(project="recommend_data_baseline2", job_type="baseline2")
    try:
        main()
        # 正常終了時の alert
        wandb.alert(
            title="recommend_data_baseline2 Creation Finished",
            text="The dataset creation process has completed successfully.",
        )
    except Exception as e:
        # エラー発生時の alert
        wandb.alert(
            title="recommend_data_baseline2 Creation Error",
