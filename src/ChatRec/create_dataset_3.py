...
def create_formatted_data(llm, data: dict) -> list:
    ...
    for rec_info, score in zip(candidates_infos, A_Scores):
        if score == 1:
            dialogue, generated_summaries = llm.generate_summaries(
                dialogue=dialogue, user_name="A", num_sentence=5
            )
    ...
    for rec_info, score in zip(candidates_infos, B_Scores):
        if score == 1:
            dialogue, generated_summaries = llm.generate_summaries(
                dialogue=dialogue, user_name="B", num_sentence=5
            )
            recommendation_sentence = llm.generate_rec_sentence_with_no_dialogue(
                rec_info
            )
...
def create_datasets_3():
    ...
    output_dir = os.path.join(BASE_DIR, "../../data/ChatRec/datasets_3")
