...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Tabidachi DeBERTaモデルの学習"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    # 環境変数とディレクトリの設定
    os.environ["WANDB_PROJECT"] = f"Tabidachi_Deberta_{TRAIN_MODEL.replace('&', '_')}"
    BEST_MODEL_DIR = f"./deberta_best_model_{TRAIN_MODEL}"
    TEST_OUTPUT_DIR = f"./deberta_{TRAIN_MODEL.replace('&', '_')}"
    
    print(f"\n{'='*60}")
    print(f"Tabidachi DeBERTaモデルの学習: {TRAIN_MODEL}")
    ...
    wandb.alert(
        title=f"Tabidachi_deberta_{TRAIN_MODEL} 学習終了",
