...
def create_recommend_data(data, llm):
    ...
    for candidate_info in candidates_info_list:
        predicted_score = predict_score_with_deberta_baseline2(
            dialogue_summary, candidate_info
        )
...
def main():
    ...
    output_dir = os.path.join(BASE_DIR, "../../data/Tabidachi/recommend_data_baseline2/")
    # 出力ディレクトリが存在しない場合は作成
        ...
        if int(filename[:3]) in TEST_ID:
            if filename[4] == "1" or filename[4] == "2" or filename[4] == "3":
                ...
                with open(input_dir + filename, "r", encoding="utf-8") as file:
                    ...
                    for data in datas:
                        output_datas.append(create_recommend_data(data, llm))
