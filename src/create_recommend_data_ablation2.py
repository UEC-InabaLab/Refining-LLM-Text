...
from oss_llm import Llama_Swallow as SumModel
import wandb
import socket
...
def many_main():
    ...
    for i in range(5):
        rec_llm = RecModel(model_name=f"dpo-recommendation-results-new_{i+1}")
        sum_llm = SumModel()
        input_dir = "../data/datasets_1/"
        output_dir = f"../data/recommend_data_ablation2_{i+1}/"
        # 出力ディレクトリが存在しない場合は作成
...
if __name__ == "__main__":
    ...
    wandb.init(project="ablation2_creation_project", job_type="create_ablation2")

    try:
        many_main()
        # 正常終了時の alert
        wandb.alert(
            title="ablation2 Creation Finished",
            text="The dataset creation process has completed successfully.",
        )
    except Exception as e:
        # エラー発生時の alert
        wandb.alert(
            title="ablation2 Creation Error",
